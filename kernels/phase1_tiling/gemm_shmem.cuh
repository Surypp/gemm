...
// --- launch wrapper ---
template <typename Tag, int BM, int BN, int BK>
void launch_gemm_shmem(GemmDescRowMajor<Tag>& desc, cudaStream_t stream = 0) {
...
#define DECL_SHMEM(TAG, BM, BN, BK) \
    extern template void launch_gemm_shmem<TAG, BM, BN, BK>(GemmDescRowMajor<TAG>&, cudaStream_t);
