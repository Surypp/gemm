...
#include "kernels/phase1_tiling/gemm_shmem.hpp"
...
// Tile64: 64*64=4096 > 1024 max threads → exceeds HIP limit.
TEST_F(Phase1Test, Tile64_1024x1024x1024) {
    GTEST_SKIP() << "Phase1 Tile64: block(64,64)=4096 threads > 1024 HIP limit (known Phase1 limitation)";
}

TEST_F(Phase1Test, Tile128_1024x1024x1024) {
    GTEST_SKIP() << "Phase1 Tile128: block(128,128)=16384 threads > 1024 HIP limit (known Phase1 limitation)";
}
