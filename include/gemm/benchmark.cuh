...
// Matches cuBLAS convention so pct_cublas_peak is comparable.
...
struct BenchmarkConfig {
    ...
    int warmup_iters = 5;
    int timed_iters  = 20;
    // sync_each=true inserts cudaDeviceSynchronize between timed iters — debug only
...
struct BenchmarkResult {
    ...
    double min_ms    = 0.0;
    double tflops    = 0.0;
    double pct_cublas_peak = 0.0;   // filled by bench_suite after cuBLAS run
};
...
inline BenchmarkResult run_benchmark(
    ...
    std::function<void(cudaStream_t)> kernel_fn,
    cudaStream_t stream = 0)
{
    ...
    for (int i = 0; i < cfg.timed_iters; ++i) {
        ...
        if (cfg.sync_each) CUDA_CHECK(cudaDeviceSynchronize());
    }
