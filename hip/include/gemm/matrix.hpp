...
struct DeviceMatrix {
        ...
        : rows(rows), cols(cols), ld(ld < 0 ? cols : ld)
    {
        ...
        HIP_CHECK(hipMemset(ptr, 0, bytes()));
    }
    ...
    void copy_from(const HostMatrix<T>& h) {
        ...
        HIP_CHECK(hipMemcpy(ptr, h.ptr(), bytes(), hipMemcpyHostToDevice));
    }

    void copy_to(HostMatrix<T>& h) const {
        assert(h.rows == rows && h.cols == cols);
        HIP_CHECK(hipMemcpy(h.ptr(), ptr, bytes(), hipMemcpyDeviceToHost));
    }

    void zero() { HIP_CHECK(hipMemset(ptr, 0, bytes())); }
};
...
HostMatrix<T> HostMatrix<T>::from_device(const DeviceMatrix<T>& d) {
    ...
    HIP_CHECK(hipMemcpy(h.ptr(), d.ptr, d.bytes(), hipMemcpyDeviceToHost));
    return h;
}
