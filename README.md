...
cuBLAS baseline (`cublasGemmEx`, `CUBLAS_TENSOR_OP_MATH`): **115.1 TFLOPS** (sq4k) · **120.4 TFLOPS** (sq8k).
...
| 4 | + cp.async double-buffer (NS=2) | **95.3** | **82.8%** |
| 5 | PTX mma.sync (manual fragment layout) | 78.9 | 68.5% |
| 6a | + ldmatrix.sync (warp-cooperative smem→reg) | 94.6 | 82.2% |
| 6b | + cp.async multi-stage (NS=2) | **91.2** | **79.3%** |
| 6c | + TMA + mbarrier (async tile load) | **87.5** | **76.0%** |

sq8k (8192×8192×8192):

| Phase | Kernel | TFLOPS | % cuBLAS |
|-------|--------|-------:|:--------:|
| 4 | cp.async double-buffer | **104.3** | **86.6%** |
| 6a | ldmatrix.sync | 102.4 | 85.0% |
| 6b | cp.async multi-stage | 99.8 | 82.9% |
| 6c | TMA + mbarrier | 99.4 | 82.5% |
